...
class IntegrationsView extends StatelessWidget {
  ...
  const IntegrationsView({required this.integrations, super.key});

  @override
  Widget build(BuildContext context) {
    final form = context.read<FormCubit>();
    return BlocBuilder<FormCubit, FormData>(builder: (context, state) {
      final values = Map<String, bool>.from(state.value<Map>('state') ?? const {});
  ...
  String _categoryLabel(IntegrationCategory category) => switch (category) { IntegrationCategory.accounting => 'Accounting', IntegrationCategory.payments => 'Payments', IntegrationCategory.operations => 'Operations' };
  Color _categoryColor(BuildContext context, IntegrationCategory category) => switch (category) { IntegrationCategory.accounting => SuperMaterialThemeData.of(context).colorScheme.primary, IntegrationCategory.payments => SuperMaterialThemeData.of(context).colorScheme.secondary, IntegrationCategory.operations => SuperMaterialThemeData.of(context).colorScheme.tertiary };
}
