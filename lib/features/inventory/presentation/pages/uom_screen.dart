...
class _UomScreenState extends State<UomScreen> {
  ...
  Widget build(BuildContext context) {
    final groups = {
      'count': ('Count', 'PCS', SuperMaterialThemeData.of(context).colorScheme.primary, [('PCS', 'Piece', '1', true), ('DZN', 'Dozen', '12', false), ('BOX', 'Box', '24', false), ('CTN', 'Carton', '144', false), ('PLT', 'Pallet', '600', false)]),
      'weight': ('Weight', 'KG', SuperMaterialThemeData.of(context).colorScheme.secondary, [('G', 'Gram', '0.001', false), ('KG', 'Kilogram', '1', true), ('TON', 'Tonne', '1000', false), ('BAG', 'Bag 50kg', '50', false)]),
      'length': ('Length', 'M', SuperMaterialThemeData.of(context).colorScheme.tertiary, [('CM', 'Centimeter', '0.01', false), ('M', 'Meter', '1', true), ('KM', 'Kilometer', '1000', false)]),
          ...
          for (final e in groups.entries)
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: _active == e.key ? superCoreTint(e.value.$3, 0x14) : SuperMaterialThemeData.of(context).superTheme.surface, border: Border.all(color: _active == e.key ? e.value.$3 : SuperMaterialThemeData.of(context).superTheme.border), borderRadius: BorderRadius.circular(10)),
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                  Eyebrow(e.value.$1, color: SuperMaterialThemeData.of(context).superTheme.fg3, size: 10),
                  const SizedBox(height: 6),
                  Text('${e.value.$4.length} units', style: TextStyle(fontFamily: SuperMaterialThemeData.of(context).textTheme.bodyMedium?.fontFamily, fontSize: 16, fontWeight: FontWeight.w700, color: e.value.$3)),
                  const SizedBox(height: 2),
                  Text('base · ${e.value.$2}', style: TextStyle(fontFamily: SuperMaterialThemeData.of(context).textTheme.bodyMedium?.fontFamily, fontSize: 11, color: SuperMaterialThemeData.of(context).superTheme.fg3)),
                ]),
              ),
            ),
        ],
