...
class StoreDetailScreen extends StatelessWidget {
  ...
  Widget _stockRow(BuildContext context, StoreStockItem item, bool last) {
    final qtyColor = switch (item.status) { StoreStockStatus.outOfStock => SuperMaterialThemeData.of(context).colorScheme.error, StoreStockStatus.lowStock => SuperMaterialThemeData.of(context).colorScheme.tertiary, StoreStockStatus.inStock => SuperMaterialThemeData.of(context).superTheme.fg1 };
    final tone = switch (item.status) { StoreStockStatus.inStock => PillTone.success, StoreStockStatus.lowStock => PillTone.warning, StoreStockStatus.outOfStock => PillTone.danger };
    final label = switch (item.status) { StoreStockStatus.inStock => 'In', StoreStockStatus.lowStock => 'Low', StoreStockStatus.outOfStock => 'Out' };
    return Container(padding: const EdgeInsets.symmetric(vertical: 12), decoration: BoxDecoration(border: last ? null : Border(bottom: BorderSide(color: SuperMaterialThemeData.of(context).superTheme.border))), child: Row(children: [
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(item.name, style: TextStyle(fontSize: 13.5, fontWeight: FontWeight.w600, color: SuperMaterialThemeData.of(context).superTheme.fg1, fontFamily: SuperMaterialThemeData.of(context).textTheme.bodyMedium?.fontFamily)), const SizedBox(height: 2), Text(item.sku, style: TextStyle(fontFamily: SuperMaterialThemeData.of(context).textTheme.bodyMedium?.fontFamily, fontSize: 11, color: SuperMaterialThemeData.of(context).superTheme.fg3))])),
      Text('${item.quantity}', style: TextStyle(fontFamily: SuperMaterialThemeData.of(context).textTheme.bodyMedium?.fontFamily, fontSize: 14, fontWeight: FontWeight.w600, color: qtyColor)),
      const SizedBox(width: 12),
