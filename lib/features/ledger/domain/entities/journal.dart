...
class LedgerAccount extends Equatable {
  ...
  const LedgerAccount({required this.code, required this.name, required this.category});
...
class JournalEntrySummary extends Equatable {
  ...
  const JournalEntrySummary({required this.reference, required this.description, required this.amount, required this.status, required this.occurredAt, this.lines = const []});

  @override
  List<Object?> get props => [reference, description, amount, status, occurredAt, lines];
}
