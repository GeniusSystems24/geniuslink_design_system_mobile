...
class RolePermissionMatrix extends Equatable {
  ...
  PermissionLevel levelFor(String module, int roleIndex) =>
      values[module]![roleIndex];

  RolePermissionMatrix cycle(String module, int roleIndex) {
    final nextValues = {
      for (final entry in values.entries)
        entry.key: List<PermissionLevel>.of(entry.value),
    };
    const levels = PermissionLevel.values;
    final current = nextValues[module]![roleIndex];
    nextValues[module]![roleIndex] =
        levels[(current.index + 1) % levels.length];
